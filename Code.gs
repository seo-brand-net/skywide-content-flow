...
function checkWebSearchAvailability() {
  ...
  if (!apiKey) {
    debugLog('AVAILABILITY_CHECK', 'ANTHROPIC_API_KEY missing in Script Properties');
    return false;
  }
  return true;
}
...
/**
 * Normalizes a header string for consistent mapping (lowercase, no extra spaces, spaces to underscores)
 */
...
/**
 * Ensures required columns exist, handling case-insensitive and space/underscore flexible checks
 */
function ensureRequiredColumns(sheet) {
  const lastCol = sheet.getLastColumn();
  let headers = [];
  if (lastCol > 0) {
    headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0].map(normalizeHeader);
  }
  
  const required = ['url', 'url_type', 'page_type', 'primary_keyword', 'secondary_keyword', 'longtail_keywords', 'location', 'intent', 'status', 'brief_url', 'run_id', 'notes'];
  
  required.forEach(req => {
    if (!headers.includes(normalizeHeader(req))) {
...
function getHeaderMap(sheet) {
  ...
  const map = {};
  headers.forEach((h, i) => {
    map[normalizeHeader(h)] = { index: i, name: h };
  });
  return map;
}
...
/***** MAIN EXECUTION *****/
function runBriefGeneration(workbookUrl, overrideFolderId) {
  ...
  debugLog('ACTIVE_WORKBOOK', { url: workbookUrl, sheetName: sheet.getName() });
  ...
  debugLog('HEADERS_DETECTED', Object.keys(headers));
  const data = sheet.getDataRange().getValues();
  const statusCol = headers['status']?.index;
  const runIdCol = headers['run_id']?.index;
  const primaryKeyCol = headers['primary_keyword']?.index;
  if (statusCol === undefined || runIdCol === undefined || primaryKeyCol === undefined) throw new Error('Missing columns: status, run_id, or primary_keyword');
  ...
  for (let r = 1; r < data.length; r++) {
    ...
    const primaryKey = String(data[r][primaryKeyCol] || '').trim();
    
    debugLog(`EVALUATING_ROW_${r}`, { status, primaryKey });
    
    // Only process rows that have a keyword and are either NEW or empty status
    if ((status === '' || status === 'NEW') && primaryKey !== '') {
      targets.push(r);
  ...
  if (targets.length === 0) {
    const samples = data.slice(1, 6).map((row, i) => {
      const obj = {};
      Object.keys(headers).forEach(k => {
        obj[k] = row[headers[k].index];
      });
    ...
    return { 
      status: "success", 
      message: "No new rows with keywords found",
      debug: {
        sheetName: sheet.getName(),
        headerMap: headers, // Full map with indices
        totalRows: data.length,
        statusColIndex: statusCol,
        primaryKeyColIndex: primaryKeyCol,
        rowSamples: samples
      }
    };
  }
...
function generateBriefWithClaude(strategy, retryCount = 0) {
  ...
  try {
    ...
    while (currentTurn < 8) { // Increased turns for deep research
      currentTurn++;
      ...
      const response = UrlFetchApp.fetch('https://api.anthropic.com/v1/messages', {
        ...
        headers: { 
          ...
          'anthropic-beta': 'web-search-2025-03-05' // REQUIRED for web_search_20250305
        },
      ...
      if (statusCode !== 200) {
        debugLog('CLAUDE_API_ERROR', { statusCode, responseText });
      ...
      if (textBlocks.length > 0) {
        ...
        if (responseData.stop_reason === 'end_turn' || (responseData.stop_reason === 'stop_sequence' && briefText.includes('}'))) {
          break;
        }
      }
...
function doPost(e) {
  try {
    ...
    if (command === 'generateDirectly') result = runClientBriefDirectly(params.folderId, params.data, workbookUrl);
      ...
      if (result) result.generation = runBriefGeneration(workbookUrl, params.folderId);
    } else result = runBriefGeneration(workbookUrl, params.folderId);
