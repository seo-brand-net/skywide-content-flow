...
## 2. Admin SQL Registration
**Problem:** SQL scripts for manual admin creation failed due to missing `provider_id` in the `auth.identities` table.
- **Corrected SQL Pattern:**
  ```sql
  -- Important: provider_id must be the string version of the user's UUID for email providers
  INSERT INTO auth.identities (id, user_id, identity_data, provider, provider_id, ...)
  VALUES (gen_random_uuid(), new_user_id, ..., 'email', new_user_id::text, ...);
  ```
