...
data.nodes.forEach((n, idx) => {
    if (n.name === 'Keyword Strategist' && n.type === 'n8n-nodes-base.code') {
        ...
        const newCode = `// Instruction Strategist v5 — Keywords + Structure + EEAT + Style
...
`;
        if (n.parameters.jsCode !== newCode) {
            n.parameters.jsCode = newCode;
            updated = true;
            console.log("✅ Updated Keyword Strategist (index " + idx + ")");
        } else {
            console.log('Keyword Strategist already updated with Phase 4/5 logic.');
        }
    }
});
