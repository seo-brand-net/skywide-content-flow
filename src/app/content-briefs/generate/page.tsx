...
export default function ContentBriefsPage() {
    ...
    usePusherBriefUpdates(selectedClient, (update) => {
        if (userRole !== 'admin' && update.user_id && update.user_id !== user?.id) {
            console.log('[Pusher] Ignoring update for another user');
            return;
        }

        console.log('[Pusher] Handling instant update:', update);

        // Instantly update the React Query cache for this client
        queryClient.setQueryData(['workbook_rows', selectedClient, userRole], (oldData: WorkbookRow[] | undefined) => {
            // Handover: if we get a Pusher event, we know the engine is active
            if (localIsGenerating && (update.status === 'NEW' || update.status === 'IN_PROGRESS')) {
                setLocalIsGenerating(false);
            }
            if (!oldData) return [update as unknown as WorkbookRow];

            // 1. Try to find existing row by keyword (match during early NEW/IN_PROGRESS) or ID
            const rowIndex = oldData.findIndex(row =>
                (update.id && row.id === update.id) ||
                (row.primary_keyword === update.primary_keyword && row.status !== 'DONE')
            );
            ...
            if (rowIndex > -1) {
                ...
                newData[rowIndex] = {
                    ...newData[rowIndex],
                    ...update,
                    // Ensure we don't overwrite if existing data is newer/DONE
                    status: (update.status as string) || newData[rowIndex].status
                };
                return newData;
            } else {
                // New row detected (e.g. from a fresh batch) - prepend it
                return [update as unknown as WorkbookRow, ...oldData];
            }
        });

        // Trigger a toast for completion/error
        if (update.status === 'DONE' || update.status === 'ERROR') {
            ...
            toast({
                title: update.status === 'DONE' ? "✨ Content Brief Generated!" : "Generation Failed",
                description: update.status === 'DONE'
                    ? `Successfully created brief for "${update.primary_keyword}"`
                    : update.notes || "An error occurred during brief generation",
                variant: update.status === 'DONE' ? "default" : "destructive",
            });

            // After a completion, we don't clear generationStartTime here anymore
            // We let isGenerating useMemo handle the transition once all rows are processed
        }
    });

    // Sync state: Is any brief currently generating for this client?
    const isGenerating = useMemo(() => {
        ...
        if (localIsGenerating && hasActiveRows) {
            setLocalIsGenerating(false);
        }

        return localIsGenerating || hasActiveRows;
    }, [workbookRows, localIsGenerating]);

    useEffect(() => {
        // Only clear start time and duration logging when everything is fully finished
        if (!isGenerating && generationStartTime) {
            const duration = Math.round((Date.now() - generationStartTime) / 1000);
            console.log(`Automation batch completed in ${duration}s`);
            setGenerationStartTime(null);
            // Final refresh to ensure all details are consistent
            queryClient.invalidateQueries({ queryKey: ['workbook_rows', selectedClient] });
        }
    }, [isGenerating, generationStartTime, queryClient, selectedClient]);
    ...
    const automationMutation = useMutation({
        ...
        onSuccess: (result) => {
            ...
            setLocalIsGenerating(true); // Force loading state immediately

            toast({
                title: "🚀 Research Engine Started",
                description: rowCount > 0
                    ? `Deep content analysis is now running for ${rowCount} keyword(s).`
                    : "Deep content analysis is now running. You'll be notified when complete.",
                variant: "default",
            });
        ...
        onError: (error: any) => {
            const isNoData = error.message?.includes('No rows with status') || error.message?.includes('NO_NEW_ROWS');

            toast({
                title: isNoData ? "ℹ️ No Action Required" : "Automation Failed",
                description: isNoData
                    ? "Your workbook doesn't have any rows with 'NEW' or empty status. Add some keywords to the Content Brief tab first!"
                    : error.message || "The research engine encountered an error.",
        ...
        <div className="min-h-screen bg-background p-8">
            <div className="max-w-screen-2xl mx-auto">
                ...
                <Button
                    ...
                    onClick={() => router.push('/content-briefs')}
                >
