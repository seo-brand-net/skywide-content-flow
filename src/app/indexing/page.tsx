                ...
                <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        ...
                        <p className="seobrand-description text-lg">
                            Submit URLs to Google &amp; Bing for clients on a 14-day automated schedule.
                        </p>
                    </div>
                                ...
                                {currentClient && (
                                    <div className="space-y-4 animate-in fade-in duration-300">
                                        ...
                                        <div className="space-y-3 p-3 bg-muted/20 rounded-lg border border-border/40 text-xs">
                                            <div>
                                                <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-0.5">GSC Property</p>
                                                <p className="font-mono text-foreground/80 break-all">{currentClient.gsc_property}</p>
                                            </div>
                                            {currentClient.bing_site_url && (
                                                <div>
                                                    <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-0.5">Bing URL</p>
                                                    <p className="font-mono text-foreground/80 break-all">{currentClient.bing_site_url}</p>
                                                </div>
                                            )}
                                            <div>
                                                <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-0.5">Last Auto Run</p>
                                                <p className="text-foreground/80">
                                                    {formatDate(currentClient.last_run_at)}
                                                </p>
                                            </div>
                                            {!currentClient.bing_site_url && (
                                                <div className="text-[10px] text-muted-foreground/60 italic">
                                                    Bing indexing disabled for this client
                                                </div>
                                            )}
                                        </div>
